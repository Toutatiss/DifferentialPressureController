...
PressureControllerSerialUI::PressureControllerSerialUI()
    : _infoTimer(_updateTime, SECONDS),
      _SDBufferTimer(500),
      _SDWriteTimer(10, SECONDS),
      _feedbackTimer(_updateTime, SECONDS),
...
void PressureControllerSerialUI::begin(int baud)
{
    ...
    if (!inDebuggingMode)
    {
        ...
        if (_pumpsResponding)
        {
            Serial.println(F("Disabling active pumps..."));
            turnAllPumpsToState(false);
        }
    }

    // Debugging response override
    if (inDebuggingMode)
    {
        Serial.println(F("### Debugging mode active. Network connection will be spoofed for 1 minute."));
        _pumpsResponding = true;
        _prevPumpsResponding = true;
        _diffSensorResponding = true;
        _prevDiffSensorResponding = true;
    }
...
{
    createSafeString(serialInput, 5);
...
void PressureControllerSerialUI::_printCurrentState()
{
    ...
    case STATE_DIFFERENTIAL:
        ...
        Serial.println(F("WARNING: Currently only compatible with BAR. Make sure the pump units are set to BAR."));
        break;
    case STATE_RESET:
        Serial.println(F("\n--- Resetting System... ---"));
        break;
...
void PressureControllerSerialUI::_updateAllInfo()
{
    ...
    if (_diffSensorResponding)
    {
        ...
        {
            _currentState = STATE_RESET;
            Serial.println(F("WARNING: Differential sensor range about to be exceeded!"));
...
void PressureControllerSerialUI::_runDifferentialController()
{
    ...
    const float Kd = 0.1;
    const float dt = _updateTime; // Time increment to integrate over

    // Calculate Error
    float error = _diffPressSensor.diffPressSetpoint - _diffPressSensor.differentialPressure; // Calculate the error

    // Calculate Integral of Error
    static float errorIntegral = 0;               // This is only initialised once
